...
int cpt_col_space(char const *str, int index)
{
    int k = 0;
    while (str[k + index] != '\0' &&
    str[k + index] != ' ' && str[k + index] != '\n') {
        k++;
    }
    return k;
}

void help_lign(char const *str, int *b, int *k)
{
    while (str[*b] == ' ' || str[*b] == '\n' || str[*b] == '\t') {
        *k = 1; *b = *b + 1;
    }
...
char **my_str_to_word_array_space(char const *str)
{
    ...
    char **tab = malloc(sizeof(char *) * (cpt_lign_space(str) + 1));
    while (i_tab < cpt_lign_space(str)) {
        while (str[i_str] == '\n' || str[i_str] == ' ' || str[i_str] == '\t') {
            i_str++;
        }
        tab[i_tab] = malloc(sizeof(char) * (cpt_col_space(str, i_str) + 1));
